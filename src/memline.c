    ...
    BUF		*buf;
{
	 ...
	 */
	home_replace(NULL, buf->b_ffname, b0p->b0_fname, B0_FNAME_SIZE);
	if (b0p->b0_fname[0] == '~')
	{
 ...
 */
	...
	fname = (char_u *)"";
    len = STRLEN(fname);
    if (len >= 4 && STRNICMP(fname + len - 4, ".sw", 3) == 0)
    {
	directly = TRUE;
 ...
 */
	...
	home_replace(NULL, curbuf->b_ffname, NameBuff, MAXPATHL);
...
#ifdef SYNTAX_HL
	if (buf->b_syn_change_lnum > lnum)
	    buf->b_syn_change_lnum = lnum;
#endif
    }
