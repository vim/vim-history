...
"       :se et|0;/^" function Justify(/+;/^$/2-g/^"/Justify 72 2
...
function Justify(...) range
	...
	if indent >= 0
		...
		let indent_str2 = '' 
		while indent_sw > 0
		    let indent_str2 = indent_str2 . indent_str
		    let indent_sw = indent_sw - 1
		endwhile
		let indent_str = indent_str2
	    endif
	endif
    else
	let indent = -1
	...
	let indent_orig = matchstr( str, '^\s*')
	...
	let str = substitute( str, '\(\S\)\s\+', '\1 ', 'g')
	...
	if str_n < tw - indent_n    " Extra spaces can be added
	    " How many spaces should be added
	    let s_add = tw - str_n - indent_n
	    let s_nr  = strlen( substitute( str, '\S', '', 'g') )
